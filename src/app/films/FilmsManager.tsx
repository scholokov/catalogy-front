...
const compareFilmsByField = (
  left: FilmCollectionItem,
  right: FilmCollectionItem,
  sortBy: SortBy,
  sortDirection: SortDirection,
) => {
...
const sortFilmCollection = (
  ...
  sorted.sort((left, right) => {
    for (const [field, direction] of sortSequence) {
      const cmp = compareFilmsByField(left, right, field, direction);
      if (cmp !== 0) return cmp;
    }
...
export default function FilmsManager({
  ...
  const logLazy = useCallback(
    (label: string, payload?: Record<string, unknown>) => {
      ...
      console.log("[lazy][films]", message);
    },
    [lazyDebugEnabled],
  );
  ...
  useEffect(() => {
    ...
    setSelectedViewImageUrls(null);
    setSelectedViewPeople(
      selectedView?.items.id && selectedViewPreloadedPeople?.itemId === selectedView.items.id
        ? selectedViewPreloadedPeople.people
        : [],
    );
    setSelectedViewGenres([]);
    setIsRefreshPickerOpen(false);
    setRefreshSearchQuery("");
  }, [selectedView?.id, selectedView?.items.id, selectedViewPreloadedPeople]);
  ...
  useEffect(() => {
    ...
    setSelectedViewImageUrls(selectedViewItemDraft?.imageUrls ?? null);
  }, [selectedView?.id, selectedViewItemDraft?.imageUrls]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [storedPeople, storedGenres] = await Promise.all([
        loadStoredPeopleForItem(supabase, selectedItemId),
        loadStoredGenresForItem(supabase, selectedItemId),
      ]);
      if (!isCancelled) {
        setSelectedViewPeople(storedPeople);
  ...
  const loadSelectedViewById = useCallback(
    async (viewId: string) => {
      ...
      return (data as FilmCollectionItem | null) ?? null;
    },
  ...
  const loadSelectedViewByItemId = useCallback(
    async (itemId: string) => {
      ...
      return ((data as FilmCollectionItem[] | null) ?? [])[0] ?? null;
    },
    [friendAccessState, ownerUserId, readOnly],
  );

  const loadSelectedFilmByItemId = useCallback(async (itemId: string) => {
    const { data } = await supabase
      .from("items")
      .select(
        "id, title, title_uk, title_en, title_original, description, genres, director, actors, poster_url, external_id, film_media_type, imdb_rating, trailers, year",
      )
      .eq("id", itemId)
      .eq("type", "film")
      .maybeSingle();
    ...
    return {
      ...
      itemId: data.id,
      title: data.title,
      englishTitle: data.title_en ?? undefined,
      ...
      mediaType: data.film_media_type ?? "movie",
  ...
  const filmViewsByExternalKey = useMemo(() => {
  ...
  const resolveAndOpenFilmDraft = useCallback(
    async (film: FilmResult) => {
      if (film.existingViewId) {
        const filmExternalKey = getFilmExternalKey(film.id, film.mediaType);
        const existingView =
          collection.find((item) => item.id === film.existingViewId) ??
          collection.find((item) => {
            const itemExternalKey = getFilmExternalKey(
              item.items.external_id,
              item.items.film_media_type,
            );
            return itemExternalKey !== null && itemExternalKey === filmExternalKey;
          });
        if (existingView) {
          await openSelectedView(existingView);
          setIsAddOpen(false);
          return true;
        }
      }
      ...
      try {
        ...
        if (detailResponse.ok) {
          ...
          openSelectedFilmDraft(detail);
          return true;
        }
      } catch {
        // Fall back to the basic search result when detail fetch fails.
      }

      openSelectedFilmDraft(film);
      return true;
    },
    [collection, openSelectedFilmDraft, openSelectedView],
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      const film = await loadSelectedFilmByItemId(requestedAddItemId);
      if (isCancelled) {
        return;
      }
      if (!film) {
    ...
    const bestMatch = sortedResults.find(
      (item) => {
        ...
        if (
          item.inCollection ||
          item.existingViewId ||
          filmViewsByExternalKey.get(itemExternalKey ?? "") ||
  ...
  const handleWatchSelectedViewTrailer = async () => {
    ...
    await watchExistingEntryTrailer({
      ...
      fetchTrailers: fetchFilmTrailers,
      openTrailerModal,
  ...
  const handleFilmSearch = async ({ query, year, director }: CatalogSearchRequest) => {
    ...
    return tmdbResults
      .map((item) => {
        const existingView = filmViewsByExternalKey.get(
  ...
  const renderActorLinks = (people?: FilmNormalizedPerson[] | null) => {
    const actors = (people ?? [])
  ...
  const renderDirectorLinks = (people?: FilmNormalizedPerson[] | null) => {
  ...
  const renderWriterLinks = (people?: FilmNormalizedPerson[] | null) => {
  ...
  const renderProducerLinks = (people?: FilmNormalizedPerson[] | null) => {
    ...
    >
      ...
      <div className={styles.resultContent}>
        ...
        <div className={styles.userMeta}>
          <span>Переглянуто: {item.is_viewed ? "так" : "ні"}</span>
          {item.is_viewed && item.viewed_at ? (
            <span>Дата перегляду: {formatViewedDate(item.viewed_at)}</span>
          ) : null}
    ...
    <div className={styles.searchBlock}>
      ...
      {isAddOpen ? (
        <CatalogSearchModal
          ...
          showDetailedSearch
          onSelect={async (film) => {
            await resolveAndOpenFilmDraft(film);
          }}

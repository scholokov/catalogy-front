...
import { buildGameProfilePromptRows, buildPlatformScopeProfilePrompt } from "@/lib/profile-analysis/game";
import {
  DEFAULT_SCOPE_MATURITY_THRESHOLDS,
  buildScopeBreakdownEntry,
  getScopeMaturityLabel,
} from "../statistics/lib/scopeReadiness";
import type { ScopeBreakdownEntry, ScopeMaturityStatus } from "../statistics/statisticsTypes";
import type {
  GameProfileAnalysisResult,
...
const buildGameAnalysisState = (
  ...
  savedRow: RawGameAnalysisRow | null,
...
const getNewGameViewsAfterAnalysis = (
  rows: GameProfileRow[],
  analyzedAt: string,
  platform: string,
) => {
  ...
  return rows.filter((row) => {
    if (!row.platforms.includes(platform) || !row.isViewed || !row.createdAt) {
      return false;
    }
...
const getProfileFreshnessLabel = (
  analysis: GameAnalysisState | undefined,
...
const renderScopeCard = (
  ...
  (() => {
    const promptRows = buildGameProfilePromptRows(rows, entry.scopeValue);
    const sourceTitlesCount = analysis?.sourceTitlesCount ?? promptRows.length;
    const newViewsCount = analysis
      ? getNewGameViewsAfterAnalysis(rows, analysis.analyzedAt, entry.scopeValue)
      : 0;
    ...
    return (
      <article key={entry.scopeValue} className={styles.nestedCard}>
        ...
        {analysis ? <ProfileAnalysisUserView mediaKind="game" analysis={analysis.userProfile} /> : null}
      </article>
    );
  })()
);

export default function ProfileGamesSection() {
  const [rows, setRows] = useState<GameProfileRow[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState("");
  const [previewModalState, setPreviewModalState] = useState<PreviewModalState | null>(null);
  const [userId, setUserId] = useState<string | null>(null);
  const [analysisByScope, setAnalysisByScope] = useState<Record<string, GameAnalysisState>>({});
  ...
  useEffect(() => {
    ...
    const loadRows = async () => {
      ...
      try {
        ...
        const pageSize = 1000;
        let from = 0;
        const collected: GameProfileRow[] = [];

        while (true) {
          const { data, error } = await supabase
            .from("user_views")
            .select(
              "created_at, is_viewed, rating, view_percent, platforms, items:items!inner(title, year, genres, type)",
            )
            .eq("user_id", user.id)
            .eq("items.type", "game")
          ...
          if (error) {
            if (!isCancelled) {
              setRows([]);
              setErrorMessage("Не вдалося завантажити профайл ігор.");
            }
            return;
          }

          const chunkRaw = (data ?? []) as RawGameRow[];
        ...
        const nextAnalysisByScope = analysisError
          ? {}
          : ((analysisData ?? []) as RawGameAnalysisRow[]).reduce<Record<string, GameAnalysisState>>(
              (accumulator, row) => {
        ...
        if (!isCancelled) {
          ...
          setAnalysisByScope({});
          setErrorMessage(
            error instanceof Error ? error.message : "Не вдалося завантажити профайл ігор.",
          );
        }
      } finally {
        if (!isCancelled) {
  ...
  const scopeEntries = useMemo(() => {
    ...
    rows.forEach((row) => {
      row.platforms.forEach((platform) => platformSet.add(platform));
    });

    return Array.from(platformSet)
      .sort((left, right) => left.localeCompare(right, "uk"))
      .map((platform) => {
        const scopedRows = rows.filter((row) => row.platforms.includes(platform));
  ...
  const handlePreviewPrompt = (entry: ScopeBreakdownEntry) => {
    ...
    setPreviewModalState({
      ...
      content: buildPlatformScopeProfilePrompt(promptRows, entry.scopeValue),
    });
  };
  ...
  const handleAnalyzeProfile = async (entry: ScopeBreakdownEntry) => {
    ...
    try {
      const response = await fetch("/api/openai/game-profile-analysis", {
        method: "POST",
        headers: {
      ...
      const optimisticAnalysis: GameAnalysisState = {
      ...
      const nextAnalysisState = buildGameAnalysisState(
        optimisticAnalysis,
        saveError ? null : (savedAnalysis as RawGameAnalysisRow),
      );
  ...
  if (isLoading) {
    return <p className={styles.message}>Завантаження профайлу ігор…</p>;
  }
  ...
  return (
    <div className={styles.content}>
      <section className={`${styles.section} ${styles.sectionFull}`}>
        <h2 className={styles.sectionTitle}>Профіль ігрових платформ</h2>
        <p className={styles.sectionText}>
          Профіль відображає окремі смакові патерни для ігрових платформ, щоб рекомендації були
          точнішими.
        </p>
      </section>

      <section className={styles.section}>
        <h3 className={styles.sectionTitle}>Готові платформи</h3>
        {workingScopes.length > 0 ? (
          ...
          <p className={styles.emptyBox}>Готових платформ для профайлу поки немає.</p>
        )}
      </section>

      {exploratoryScopes.length > 0 ? (
        <section className={styles.section}>
          <h3 className={styles.sectionTitle}>Попередні платформи</h3>
          <div className={styles.list}>

...
type DirectorDetail = {
  ...
  profileUrl: string;
  imageUrls: string[];
  filmography: DirectorFilmographyEntry[];
};

type DirectorFilmographyCard = {
  id: string;
  mediaType: "movie" | "tv";
  title: string;
  originalTitle: string;
...
const getFilmographyCardKey = (entry: DirectorFilmographyCard) =>
  [entry.mediaType, entry.id].join(":");

const mergeDirectorFilmography = (
  entries: DirectorFilmographyEntry[],
): DirectorFilmographyCard[] => {
  const grouped = new Map<string, DirectorFilmographyCard>();

  entries.forEach((entry) => {
    const key = `${entry.mediaType}:${entry.id}`;
    const creditLabel = entry.job || "Режисура";
...
const isDirectorCredit = (entry: DirectorFilmographyEntry) => {
...
export default function DirectorDetailPage({ directorId }: { directorId: string }) {
  ...
  const [message, setMessage] = useState("Завантаження режисера…");

  useEffect(() => {
    let isCancelled = false;

    void (async () => {
      setMessage("Завантаження режисера…");
      try {
        const response = await fetch(`/api/tmdb/person/${directorId}`);
        const data = (await response.json()) as DirectorDetail & { error?: string };
        if (!response.ok) {
          if (!isCancelled) {
            setDetail(null);
            setMessage(data.error ?? "Не вдалося завантажити сторінку режисера.");
          }
          return;
        }
        ...
        if (!isCancelled) {
          setDetail({
            ...data,
            filmography: data.filmography.filter(isDirectorCredit),
          });
          setMessage("");
        }
      } catch (error) {
        if (!isCancelled) {
          ...
          setMessage(
            error instanceof Error
              ? error.message
              : "Не вдалося завантажити сторінку режисера.",
          );
        }
      }
    })();
  ...
  const mergedFilmography = useMemo(
    () => mergeDirectorFilmography(detail?.filmography ?? []),
    [detail],
  );
  ...
  return (
    <CatalogLayout
      title={detail?.name ?? "Режисер"}
      description="Режисерська фільмографія з перетином із твоєю колекцією фільмів."
      headerRight={
        <Link href="/directors" className="btnBase btnSecondary">
          Пошук режисерів
        </Link>
      }
    >
          ...
          <>
              ...
              <div className={styles.heroContent}>
                <div className={styles.kpiGrid}>
                  <div className={styles.kpiCard}>
                    <span className={styles.kpiLabel}>Всього режисерських робіт</span>
            ...
            <section className={styles.section}>
              ...
              {inCollection.length > 0 ? (
                    ...
                    >
                      ...
                      <div className={styles.filmBody}>
                        ...
                        {entry.creditLabels.length > 0 ? (
                          <p className={styles.filmMeta}>
                            {entry.creditLabels.length > 1 ? "Кредити" : "Кредит"}:{" "}
                ...
                <p className={styles.emptyBox}>У колекції поки немає фільмів цього режисера.</p>
              )}
            </section>
            ...
            <section className={styles.section}>
              ...
              {notInCollection.length > 0 ? (
                ...
                <p className={styles.emptyBox}>
                  Уся доступна режисерська filmography уже перетинається з колекцією.
                </p>
              )}
            </section>
          </>

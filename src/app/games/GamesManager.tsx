...
export default function GamesManager({
  ...
  const restoreSnapshotRef =
    useRef<CatalogScreenSnapshot<Filters, GamesViewMode> | null>(null);
  ...
  useEffect(() => {
    ...
    if (snapshot) {
      ...
      if (GAMES_VIEW_MODES.includes(snapshot.viewMode)) {
  ...
  useEffect(() => {
    ...
    if (snapshotViewMode && GAMES_VIEW_MODES.includes(snapshotViewMode)) {
  ...
  const fetchPage = useCallback(async (pageIndex: number, filters: Filters) => {
    ...
    if (error) {
      ...
      const hasTotalCount = totalCountRef.current > 0;
  ...
  useEffect(() => {
    ...
    const handleCollectionEntryDeleted = (event: Event) => {
      ...
      if (detail?.mediaKind !== "game" || !detail.viewId) {
        return;
      }
        ...
        >
          ...
          >
            ...
            {visiblePlatforms.length > 0 ? (
              <FilterMultiSelectDropdown
                title="Платформи"
                options={visiblePlatforms}
                selectedValues={pendingFilters.platforms}
                allSelected={pendingFilters.platformsAll}
                emptyLabel="Оберіть платформи"
                onChange={({ selectedValues, allSelected }) =>
                  setPendingFilters((prev) => ({
                    ...prev,
                    platforms: selectedValues,
                    platformsAll: allSelected,
                  }))
                }
              />
            ) : null}

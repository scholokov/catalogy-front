...
export default function AppShell({ children }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    const loadPendingCount = async () => {
      ...
      if (!user) {
        if (!isCancelled) setFriendsPendingCount(0);
        return;
      }
    ...
    window.addEventListener("friends:pending-count-refresh", handlePendingRefresh);

    return () => {
      isCancelled = true;
      window.removeEventListener("friends:pending-count-refresh", handlePendingRefresh);
      if (channel) {

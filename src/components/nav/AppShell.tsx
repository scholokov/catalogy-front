...
export default function AppShell({ children }: AppShellProps) {
  ...
  useEffect(() => {
    if (isAuthRoute || !hasSession) return;
    let isCancelled = false;
    let channel: ReturnType<typeof supabase.channel> | null = null;

    const loadFriendsBadgeCount = async () => {
      const {
        data: { user },
      } = await supabase.auth.getUser();
      if (!user) {
        if (!isCancelled) setFriendsBadgeCount(0);
        return;
      }
      const count = await loadFriendsBadgeCountValue(supabase, user.id);
      if (!isCancelled) {
    ...
    void (async () => {
      await loadFriendsBadgeCount();
      const {
        data: { user },
      } = await supabase.auth.getUser();
      if (!user || isCancelled) return;
      channel = supabase
        .channel("friends-badge-count")
        .on(
          "postgres_changes",
          {
            event: "*",
    ...
    window.addEventListener("friends:pending-count-refresh", handleBadgeRefresh);
    window.addEventListener("friends:activity-count-refresh", handleBadgeRefresh);

    return () => {
      isCancelled = true;
      window.removeEventListener("friends:pending-count-refresh", handleBadgeRefresh);
      window.removeEventListener("friends:activity-count-refresh", handleBadgeRefresh);
      if (channel) {
        void supabase.removeChannel(channel);
      }
    };
  }, [hasSession, isAuthRoute]);
      ...
      <div className="appShell">
        ...
        <div className="navHeader">
          ...
          <button
            ...
            aria-label={isNavOpen ? "Закрити меню" : "Відкрити меню"}
        ...
        <div className={`navLinks ${isNavOpen ? "navLinksOpen" : ""}`}>
          ...
          >
            ...
            <span className="navLabel">Ігри</span>
          </Link>
          <Link
          ...
          >
            ...
            {friendsBadgeCount > 0 ? (
              <span className="navBadge">{friendsBadgeCount}</span>
            ) : null}
          </Link>
          <Link
